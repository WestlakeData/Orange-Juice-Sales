...
The `cv.glmnet` function below does that work by finding THEBESTWAYTOSUCCINCTLYDESCRIBEIT, ultimately printing out coefficients for each variable that have been penalized or nullified if their relationship to other variables is multicollinear.
...
ggplot2::ggplot(df, aes(LoyalCH, PriceDiff, color=Purchase)) + geom_hline(yintercept = 0) + geom_point() + scale_color_discrete(name='Purchased MM', labels=c("No","Yes")) + labs(x='Brand Loyalty (Citrus Hill)', y='Price Difference', title = 'Observed Customer Behavior') + theme(axis.line.y = element_line(color = 'black'), panel.background = element_blank(), legend.key=element_rect(fill = 'white'))
```
...
Both the logistic model and the explanatory analysis supporting the gradient boosted trees model give us insight into the predictor variables which influence the purchase of Minute Maid orange juice by our customers.
...
ggplot2::ggplot(df, aes(PctDiscMM, color=Purchase)) + geom_density() + scale_color_discrete(name='Purchased MM', labels=c("No","Yes")) + labs(title = 'Observed Customer Behavior')
```
