...
############ POSSIBLE PCA SOLUTION ###############

df_stan <- df[-1]
df.pca <- prcomp(df_stan, scale = TRUE)

eigenvectors <- df.pca$rotation #Eigenvectors are contained in the $rotation 
eigenvectors.PC1_PC4 <- eigenvectors[,1:4] #First 4 Principal Componenets
df_matrix <- df[-1] %>% scale() %>% as.matrix() # Convert to Matrix for multiplication

pca_oj <- df_matrix %*% eigenvectors.PC1_PC4 # Multiply standardized data matrix by eigenvectors
round(cor(pca_oj), 3) #Check to see if multicollinearity was resolved

df_pca <- cbind(df[1],pca_oj) # Rebuild dataset after eigenvalue transformation

#Split PCA data
purchase_testtrain <- initial_split(df_pca, prop = 0.75, strata = Purchase) 
pca.train <- training(purchase_testtrain)
pca.test <- testing(purchase_testtrain)


mod3<- glm(Purchase ~ ., data = pca.train,family=binomial(link='logit')) # Logistic Regression using PCA data
summary(mod3)
