...
```{r logreg - preproc1, warning=FALSE, echo=TRUE}
...
```{r logreg - preproc2, warning=FALSE, echo=TRUE}
...
The `cv.glmnet()` function below applied that method to our training data set, printing out coefficients for each variable that have been penalized or nullified if their relationship to other variables is multicollinear.
...
```{r Lasso logistic regression, warning=FALSE, echo=TRUE}
...
```{r Lasso logistic regression 2, warning=FALSE, echo=TRUE}
...
The AUC tells us how optimally our model can handle the balance between true and false positives at different classification thresholds, but we will ultimately need to choose the optimal threshold for our model.

The analysis below lets us know that the optimal classification threshold for our model is 46.5% — any customer with that high or higher a likelihood of purchasing MinuteMaid should be classified as "Yes; Will Purchase MinuteMaid." That percentage optimally balances the likelihood of true positives and the risk of false positives.
...
acc_matrix <- accuracy(test$lass_preds, test$Purchase)
...
plot(acc_matrix,colorize=FALSE, col="black") 
...
```{r variable importance, warning=FALSE, echo=TRUE}
...
```{r Brand Loyalty PDP, echo=TRUE, warning=FALSE}
...
```{r LoyaltyCH vs PriceDiff plot, echo=TRUE, warning=FALSE}
...
ggplot2::ggplot(df, aes(PriceDiff, color=Purchase)) + geom_density() + scale_color_discrete(name='Purchased MM', labels=c("No","Yes")) + labs(title = 'Density plot of Observations for PriceDiff')
